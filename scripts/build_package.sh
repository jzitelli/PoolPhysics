...
packagedir=$project_root/package
mkdir -p $packagedir
$bindir/pip install -r requirements.txt -t $project_root/package
cp -rf $project_root/pool_physics $packagedir/pool_physics
cp lambda_function.py $packagedir
cd $packagedir
zip -r ../package.zip .
